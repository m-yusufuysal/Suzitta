# -*- coding: utf-8 -*-
"""
generate_3267_database.py
Generates a 3,267+ word CEFR Learning Database across 110 lessons (22 topics per level, A1 to C1).
Integrates DeepSeek API cached topic data and enriches each topic to achieve 3,267+ authentic,
topic-matched Turkish words with TR, AR, EN translations, sentences, cognates, and Mind Palace mnemonics.
"""
...
print("Initializing 3,267+ CEFR Database Generator (110 Topics, A1 to C1)...")

# 110 LESSON DEFINITIONS (22 PER LEVEL)
LEVELS_META = {
    1: {
        "cefr": "A1",
...
# Load cached deepseek topics
cache_data = {}
if os.path.exists(CACHE_FILE):
    try:
        with open(CACHE_FILE, "r", encoding="utf-8") as f:
            cache_data = json.load(f)
        print(f"Loaded existing DeepSeek cache with {len(cache_data)} topics.")
    except Exception as e:
        print("Cache load error:", e)

def fetch_topic_from_deepseek(cefr, t_tr, t_ar, t_en, count=30):
    prompt = f"""You are a Turkish language expert and lexicographer.
Provide exactly {count} authentic, highly accurate Turkish words or expressions that DIRECTLY match the topic: '{t_tr}' (CEFR Level {cefr}).

Return a JSON object containing a key "words" which is an array of {count} objects. Each object MUST have:
1. "tr": Turkish word or idiom (must be real, correct Turkish)
...
Ensure all words are 100% appropriate for '{t_tr}'.
...
print("\nEnsuring all 110 topics are populated with 30 words each...")
...
for lvl_id in range(1, 6):
    ...
    for idx, les in enumerate(lvl_info["lessons"]):
        ...
        if len(existing) >= 28:
            print(f"[{lvl_id}.{idx+1}/110] '{t_tr}' has {len(existing)} words (OK).")
            continue

        print(f"[{lvl_id}.{idx+1}/110] Fetching 30 words for '{t_tr}' (Level {cefr})...")
        new_words = fetch_topic_from_deepseek(cefr, t_tr, t_ar, t_en, count=30)
        if new_words and len(new_words) >= 15:
            cache_data[topic_key] = new_words
            with open(CACHE_FILE, "w", encoding="utf-8") as f:
                json.dump(cache_data, f, ensure_ascii=False, indent=2)
            print(f" -> Success! {len(new_words)} words cached for '{t_tr}'.")
        else:
            print(f" -> Warning: Failed or partial for '{t_tr}'.")
        time.sleep(0.4)
...
for lvl_id in range(1, 6):
    ...
    for idx, les_meta in enumerate(lvl_meta["lessons"]):
        ...
        for item in raw_words:
            ...
            s_en = item.get("sentence_en") or f"{tr_w} carries a great meaning in sentence."
...
print(f"DONE! Generated database with {len(vocab_list)} unique items across {len(levels_definition)} levels and 110 topics!")
